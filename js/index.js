...
var index = {
    ...
    ready_init: function() {
        var self = this;
        var $body = $('body');
        var url = 'http://121.42.187.170:8081/dp/hos/';
        var verify = self.search_location('verify');
        $('.hospital-content').html('');
        if (verify === 'login') {
            var hospital_url = 'http://121.42.187.170:8081/dp/hos/get';
            ...
            self.dp_get( hospital_url,{ user_id: user_id }, '', function(result) {
                if (result.success) {
                    ...
                    if (result.data.length === 1) {
                        ...
                        window.location.href = "http://121.42.187.170:8080/dp_test/";
                    }
                }
            });
        }
    ...
    hospital_list_focus: function($obj) {
        ...
        token = self.getCookie('token');
        user_id = self.getCookie('user_id');
        self.setCookie('hospital_name', name, '/dp_test', '121.42.187.170');
        self.setCookie('hospital_id', id, '/dp_test', '121.42.187.170');
        self.setCookie('token', token, '/dp_test', '121.42.187.170');
        self.setCookie('user_id', user_id, '/dp_test', '121.42.187.170');
    },
